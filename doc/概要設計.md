...
* **入力**
  ...
  * `granularity`: 粒度（行政区/統計区/JCC-JCG）。既定は `admin`。
...
* **出力**
  ...
  ```json
  {
    "results": [ { "ref?": "string", "code": "string", "name": "string" } ],
    "failed":  [ { "index": number, "ref?": "string", "code": "OUT_OF_RANGE|INVALID_ARGUMENT" } ],
    "attribution": "string"
  }
  ```

  * **入力順＝出力順**を保証。`results[i]` は `points[i]` に対応。
  * `ref` が渡されていれば出力にも **そのまま返す**（LLM側で時刻や元行とJOIN可能）。
...
3. **結果整形**：入力順を保持、`ref` をエコーバック、`results` と `failed` に分ける。
